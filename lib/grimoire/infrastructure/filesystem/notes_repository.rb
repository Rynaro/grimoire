...
module Grimoire
  module Infrastructure
    module Filesystem
      # Concrete repository that persists notes within a directory hierarchy.
      class NotesRepository
        include Grimoire::Domain::NotesRepository
        ...
        def list_notes
          Dir.glob(root.join("**/*#{NOTE_EXT}")).sort.map { |path| domain_note_for(path) }
        end
        ...
        def find_note(relative_path)
          ...
          raise Grimoire::NoteNotFoundError, relative_path unless path.exist?

          domain_note_for(path)
        end

        def create_note(relative_path, content = nil, title: nil)
          cleaned = append_extension(sanitize(relative_path))
          note = domain_note_for(root.join(cleaned))
          raise Grimoire::UnsafeOperationError, "Note already exists: #{note.id}" if note.exists?
        ...
        def create_folder(relative_path)
          resolve_folder(relative_path, create: true)
        end

        def delete_folder(relative_path)
          folder = resolve_folder(relative_path)
          raise Grimoire::UnsafeOperationError, "Refusing to delete root" if folder == root
        ...
        def resolve_link(reference, current_folder: ".")
          ...
          return domain_note_for(local_candidate) if local_candidate.exist?

          slug = slugify(reference)
          matches = Dir.glob(root.join("**/#{slug}#{NOTE_EXT}")).sort
          return nil if matches.empty?
          return domain_note_for(matches.first) if matches.one?
        ...
        def domain_note_for(path)
          Grimoire::Domain::Note.new(path, root:)
        end
        ...
        def ensure_within_root!(path)
          ...
          raise Grimoire::UnsafeOperationError, "Path escapes root: #{path}"
        end

        def cleanup_empty_parents(start_path)
          path = Pathname(start_path)
          while path != root && path.exist? && path.children.empty?
            path.rmdir
            path = path.parent
          end
